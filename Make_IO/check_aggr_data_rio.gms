...
$setglobal set_name japan_2011_40x40_rio
* $setglobal set_name japan_2011_39x39_alt
* $setglobal set_name japan_2011_37x37_new
* $setglobal set_name japan_2015_25x16
...
;
chk_row(row,"value") = sum(col,iotable(row,col)) + sum(fd,iotable(row,fd));
chk_row(va,"value") = sum(col,iotable(va,col)) + sum(fd,iotable(va,fd));
chk_row("Total","value") = sum(row, chk_row(row,"value")) + sum(va, chk_row(va,"value"));

chk_col("value",col) = sum(row,iotable(row,col)) + sum(va,iotable(va,col));
chk_col("value",fd) = sum(row,iotable(row,fd)) + sum(va,iotable(va,fd));
...
chk_row_co2(row,"value") = sum(col,iotable_co2(row,col)) + sum(fd,iotable_co2(row,fd));
chk_row_co2(va,"value") = sum(col,iotable_co2(va,col)) + sum(fd,iotable_co2(va,fd));
chk_row_co2("Total","value") = sum(row, chk_row_co2(row,"value")) + sum(va, chk_row_co2(va,"value"));

chk_col_co2("value",col) = sum(row,iotable_co2(row,col)) + sum(va,iotable_co2(va,col));
chk_col_co2("value",fd) = sum(row,iotable_co2(row,fd)) + sum(va,iotable_co2(va,fd));
chk_col_co2("value","Total") = sum(col, chk_col_co2("value",col)) + sum(fd, chk_col_co2("value",fd));

chk_data("col_sum","value") = round(chk_col("value","Total") / 1000, 1);
chk_data("row_sum","value") = round(chk_row("Total","value") / 1000, 1);
chk_data("fd_sum","value") = round(chk_va_fd("fd","value") / 1000, 1);
chk_data("va_sum","value") = round(chk_va_fd("va","value") / 1000, 1);
chk_data("col_co2_sum(MtCO2)","value") = round(chk_row_co2("Total","value"), 1);
chk_data("row_co2_sum(MtCO2)","value") = round(chk_col_co2("value","Total"), 1);

display chk_row, chk_col, chk_row_out, chk_col_out, chk_va_fd, chk_row_co2, chk_col_co2, chk_data;

chk_va(col,"value") = round(sum(va,iotable(va,col)) / 1000, 1) + eps;
chk_va("Total","value") = sum(col, chk_va(col,"value"));
chk_va(col,"sh(%)") = round(100 * chk_va(col,"value") / chk_va("Total","value"), 1) + eps;

chk_fd(row,"value") = round(sum(fd,iotable(row,fd)) / 1000, 1) + eps;
chk_fd("Total","value") = sum(row, chk_fd(row,"value"));
chk_fd(row,"sh(%)") =
    round(100 * chk_fd(row,"value") / chk_fd("Total","value"), 1) + eps;
...
chk_co2_use(col,"value") = round(sum(row, iotable_co2(row,col)), 1) + eps;
chk_co2_use(fd,"value") = round(sum(row, iotable_co2(row,fd)), 1) + eps;
...
chk_co2_use(col,"sh_(%)")$(chk_co2_use("Total","value") > 1e-6)
    = round(100 * chk_co2_use(col,"value") / chk_co2_use("Total","value"), 1) + eps;
chk_co2_use(fd,"sh_(%)")$(chk_co2_use("Total","value") > 1e-6)
    = round(100 * chk_co2_use(fd,"value") / chk_co2_use("Total","value"), 1) + eps;
